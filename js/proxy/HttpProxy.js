...
(function (proxy) {
    class HttpProxy {
        ...
        static req_repository_directory_items(domain, project, directory, group) {
            ...
            http.send(GameConfig.Substance_Directory_Items_URL, JSON.stringify(params), (data) => {
        ...
        // 物质参数更改
        static req_substance_args_change(substanceExtra) {
            let params = {};
            params["token"] = managers.LoginManager.inst.account.token;
            ...
            http.send(GameConfig.Substance_File_Args_Change_URL, JSON.stringify(params), (data) => {
                ...
                if (substanceExtra && substanceExtra.substance.id == json["sbs_id"]
                    && substanceExtra.objId == json["obj_id"] && substanceExtra.meshName == json["submesh"]) {
                    managers.LoginManager.inst.account.task = json["task"];
                    substanceExtra.textures.clear();
                    let textures = data["output_image"];
                    if (textures) {
                        for (let pro in textures) {
                            substanceExtra.textures.set(pro, textures[pro]);
                        }
                    }
                }
            });
